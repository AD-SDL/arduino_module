...
/* *
 * Interpolate write so that the robot does not jump to a position
 */
unsigned int _Braccio::iPwrite(Servo &myServo,int angle,int previousAngle) {

	Serial.println("In iPwrite");
	int SPEED = 60;
	myAngle.go(previousAngle);
	uint32_t duration = 1000.;
	myAngle.go(angle,duration,LINEAR);

	Serial.println(angle);
	Serial.println(previousAngle);

	int val = myAngle.update();
	float t = 0;
	int x = 0;	
    int num_points = 10;	
	for(int j = 0; j < num_points ;j++)
	{
            t = (j + 1.0) / (num_points + 1.0);  //Parameter for interpolation
            x = (1 - t) * previousAngle + t * angle;
...
int _Braccio::ServoMovement(int stepDelay, int vBase, int vShoulder, int vElbow,int vWrist_ver, int vWrist_rot, int vgripper) {
	...
	while (exit) 
	{		
	        //Serial.println("In servo movement ");	
